...
use crate::engine::{
    ...
    rclone_command, remove_mount_config, remote_target, reset_mount_start,
    unmount_derived_bind,
};
...
pub async fn mount_get(
    ...
    let m = mount(&conn, &id)?;
    drop(conn);
    acl(&s, &client, &m.remote_id, "file.read", &m.remote_path)?;
    Ok(Json(m))
}

pub async fn mount_delete(
    State(s): State<AppState>,
    h: HeaderMap,
    Path(id): Path<String>,
) -> Result<StatusCode> {
    ...
    db(&s)?.execute("DELETE FROM mount_profile WHERE id=?", params![id])?;
    audit(
        &s,
        Some(&c),
        "mount.delete",
...
pub async fn mount_update(
    ...
    if matches!(m.status.as_str(), "STARTING" | "RUNNING" | "STOPPING") {
        return Err(GatewayError::Message(
            "mount is currently active, please stop it before editing".into(),
        ));
    }

    validate_identity(&i.name, "mount name", 128)?;
    valid_mount(&i.mount_point)?;
    let conflict: Option<String> = db(&s)?
        .query_row(
            "SELECT id FROM mount_profile WHERE mount_point=? AND status IN ('STARTING','RUNNING','STOPPING') AND id != ?",
            params![i.mount_point, id],
            |r| r.get(0),
        )
        .optional()?;
    if conflict.is_some() {
    ...
    let cache_dir = i.cache_dir.clone().unwrap_or_else(|| {
        m.cache_dir.clone().unwrap_or_else(|| {
            s.root
                .join("cache")
                .join(format!("mount-{}", id))
                .display()
                .to_string()
        })
    });
    ...
    db(&s)?.execute(
        ...
        params![
            ...
            i.mount_point,
            cache_dir,
            i.read_only.unwrap_or(false) as i64,
            cache_mode,
            cache_max_size,
            cache_max_age,
            t,
